...
def build_report(rows: list[dict], raw: list[dict], offsets: list[dict], branches: list[dict], tasks: list[dict[str, str]], job_ids: str) -> str:
    ...
    return "\n".join([
        ...
        f"- Raw MAE: A-S={raw_by['AS_direct']['MAE_eV']:.6f} eV; triad-min={raw_by['triad_min']['MAE_eV']:.6f} eV.",
        f"- Raw trends: A-S Pearson={raw_by['AS_direct']['Pearson_r']:.6f}, Spearman={raw_by['AS_direct']['Spearman_rho']:.6f}; triad-min Pearson={raw_by['triad_min']['Pearson_r']:.6f}, Spearman={raw_by['triad_min']['Spearman_rho']:.6f}.",
        f"- Offset-only MAE: A-S={offset_by['AS_direct']['MAE_after_offset_ev']:.6f} eV; triad-min={offset_by['triad_min']['MAE_after_offset_ev']:.6f} eV.",
