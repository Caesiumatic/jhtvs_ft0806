...
#$ -cwd
#$ -j y
#$ -o runs/chauhan_cation_eox_unconstrained/scheduler_logs/$JOB_ID.$TASK_ID.out
#$ -t 1-72
...
python workflows/chauhan_cation_eox/run_calculation.py \
  --manifest data/chauhan_cation_eox/unconstrained/calculation_manifest.csv \
  --task-index "$TASK_INDEX" \
  --run-root runs/chauhan_cation_eox_unconstrained
